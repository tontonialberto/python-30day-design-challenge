...
def process_payment_credit_card(total: Decimal) -> None:
...
def process_payment_apple(total: Decimal) -> None:
...
PAYMENT_PROCESSORS: Dict[PaymentType, PaymentProcessor] = {
    PaymentType.CREDIT_CARD: process_payment_credit_card,
    PaymentType.PAYPAL: process_payment_paypal,
    PaymentType.APPLE_PAY: process_payment_apple,
}
...
def main() -> None:
    ...
    input_payment_type = input(
        "What payment method would you like to use? (cc/paypal/apple) "
    )

    try:
        payment_type = PaymentType(input_payment_type)
    except ValueError:
        print(f"Unknown payment type '{input_payment_type}'")
        exit(1)

    payment_processor: PaymentProcessor = PAYMENT_PROCESSORS[payment_type]
    cart.process_payment(payment_processor)
