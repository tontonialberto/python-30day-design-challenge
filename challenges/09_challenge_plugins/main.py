...
from factory import get_all_available_payment_method_names, register_payment_handler, payment_handlers
...
def handle_payment(total: Decimal) -> None:
    payment_type = input(
        f"What payment method would you like to use? ({'/'.join(get_all_available_payment_method_names())}) "
    )
...
def find_payment_handler_plugins(root_path: str) -> List[str]:
    """
    Payment handlers discovery. Return names of the discovered modules.
    
    Args:
    - root_path: The path to the plugins folder.
    """
    names: List[str] = []
    package_name = Path(root_path).name
    for _, dirs, files in os.walk(root_path):
        dirs.clear() # only visit the first level
        names.extend([f"{package_name}.{Path(file).stem}" for file in files])
    return names


def main() -> None:
    register_payment_handler("cc", process_payment_cc)
    register_payment_handler("paypal", process_payment_apple_pay)
    register_payment_handler("apple", process_payment_apple_pay)
    plugins_folder = os.path.abspath(os.path.join(".","payment_processors"))
    load_plugins(find_payment_handler_plugins(plugins_folder))
